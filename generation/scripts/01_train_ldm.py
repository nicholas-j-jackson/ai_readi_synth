...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def __init__(self,
                 batch_size, data_path, num_workers,
                 *args, **kwargs):
        super().__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers

        if not kwargs['pre_embed']:
        ...
        #print('age', self.train_dataset.df[['age']].mean().values[0], self.val_dataset.df[['age']].mean().values[0])
        #print('GL', self.train_dataset.df[['GL']].mean().values[0], self.val_dataset.df[['GL']].mean().values[0])
        #print('DR', self.train_dataset.df[['DR']].mean().values[0], self.val_dataset.df[['DR']].mean().values[0])
        #print('AMD', self.train_dataset.df[['AMD']].mean().values[0], self.val_dataset.df[['AMD']].mean().values[0])
        #exit()
...
if __name__ == "__main__":
    ...
    if opt.resume:
        ...
        if os.path.isfile(opt.resume):
            ...
            logdir = "/".join(paths[:-2])
            print("Resuming training in: {}".format(logdir))
            ckpt = opt.resume
        else:
